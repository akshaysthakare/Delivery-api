      ...
      <div class="box box-solid">
        ...
        <div class="box-body">
          <dl class="dl-horizontal">
            ...
            <dd><?= h($thumbnail->name) ?></dd>
            <dt scope="row"><?= __('File Name') ?></dt>
            <dd><?= h($thumbnail->file_name) ?></dd>
            <dt scope="row"><?= __('Mime Type') ?></dt>
            <dd><?= h($thumbnail->mime_type) ?></dd>
            <dt scope="row"><?= __('Extension') ?></dt>
            <dd><?= h($thumbnail->extension) ?></dd>
            <dt scope="row"><?= __('Path') ?></dt>
            <dd><?= h($thumbnail->path) ?></dd>
            <dt scope="row"><?= __('Url') ?></dt>
            <dd><?= h($thumbnail->url) ?></dd>
            <dt scope="row"><?= __('Image') ?></dt>
            <dd><?= $thumbnail->has('image') ? $this->Html->link($thumbnail->image->name, ['controller' => 'Images', 'action' => 'view', $thumbnail->image->id]) : '' ?></dd>
            <dt scope="row"><?= __('Id') ?></dt>
            <dd><?= $this->Number->format($thumbnail->id) ?></dd>
            <dt scope="row"><?= __('Size') ?></dt>
            <dd><?= $this->Number->format($thumbnail->size) ?></dd>
            <dt scope="row"><?= __('Created') ?></dt>
            <dd><?= h($thumbnail->created) ?></dd>
            <dt scope="row"><?= __('Modified') ?></dt>
            <dd><?= h($thumbnail->modified) ?></dd>
          </dl>
        </div>
      </div>
    </div>
      ...
      <div class="box box-solid">
        ...
        <div class="box-body">
          ...
          <table class="table table-hover">
              ...
              <?php foreach ($thumbnail->orders as $orders): ?>
              <tr>
                    <td><?= h($orders->id) ?></td>
                    <td><?= h($orders->sku) ?></td>
